...
function getRotationMatrix(theta, u, v, w, a, b, c, sec: ISecurityDomain) {
  ...
  return Matrix3D.axClass.FromArray([
    u2 + (v2 + w2) * cos,
    u * v * (1 - cos) + w * sin,
    u * w * (1 - cos) - v * sin,
    0,
...
export class Matrix3D extends ASObject {
  ...
  setTargetDisplayObject(object: DisplayObject): void {
  ...
  resetTargetDisplayObject(): void {
    release || assert(this._displayObject);
  ...
  clone(): Matrix3D {
    return Matrix3D.axClass.FromArray(this._matrix);
  }
  ...
  copyRowTo(row: number /*uint*/, vector3D: Vector3D): void {
  ...
  copyColumnTo(column: number /*uint*/, vector3D: Vector3D): void {
  ...
  copyRowFrom(row: number /*uint*/, vector3D: Vector3D): void {
    row = row >>> 0; vector3D = vector3D;
    var offset = row | 0;
    var m = this._matrix;
    m[offset] = vector3D.x;
  ...
  copyColumnFrom(column: number /*uint*/, vector3D: Vector3D): void {
    column = column >>> 0; vector3D = vector3D;
    var offset = column << 2;
    var m = this._matrix;
    m[offset] = vector3D.x;

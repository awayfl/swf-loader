...
import { ImageType, roundToMultipleOfFour, Inflate } from '@awayjs/graphics';
import { ImageDefinition } from './image';
//import notImplemented = Shumway.Debug.notImplemented;
//import assertUnreachable = Shumway.Debug.assertUnreachable;
//import roundToMultipleOfFour = Shumway.IntegerUtilities.roundToMultipleOfFour;

import { BitmapTag } from '../../../factories/base/SWFTags';
	...
	if (hasAlpha) {
		for (let y = 0; y < height; y++) {
			for (let x = 0; x < width; x++) {
				...
				view[i++] = (
					a << 24
					| (bytes[offset + 2] * f | 0) << 16
					| (bytes[offset + 1] * f | 0) << 8
					| (bytes[offset + 0] * f | 0)) >>> 0;
			}
			p += padding;
		}
	} else {
...
function parse24BPP(tag: BitmapTag): Uint8ClampedArray {
	...
	// ?
	// Unpremultiply
	// ex: this is wrong, because when we devide on alpha PMA - we already miss a data
	// you should use it as PMA
...
export function defineBitmap(tag: BitmapTag): {definition: ImageDefinition; type: string, id: number} {
	...
	const def =  {
		definition: {
			...
			data: null,
			dataType: type,
			// insert new field to avoid require a @awayf/graphics
			isPMA: type === ImageType.PremultipliedAlphaARGB,
			image: null
		},
		needParse: true,
		lazyParser () {

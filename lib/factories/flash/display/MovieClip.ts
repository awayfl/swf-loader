  ...
  private _startSounds(frameNum) {
    ...
    if (starts) {
      ...
      for (var i = 0; i < starts.length; i++) {
        ...
        if (!sound) {
          var symbolInfo = <SoundSymbol>loaderInfo.getSymbolById(symbolId);
          if (!symbolInfo) {
            continue;
          }
...
export class MovieClip extends Sprite implements IAdvancable {
  ...
  private _addAvm1FrameScripts(frameIndex: number,
                                actionsBlocks: ActionBlock[]): void {
  ...
  private _addAvm1InitActionBlocks(frameIndex: number,
                                    actionsBlocks: InitActionBlock[]): void
  {
  ...
  _addFrame(frameInfo: any) {
    var spriteSymbol = <SpriteSymbol><any>this._symbol;
  ...
  _enqueueFrameScripts() {
    ...
    if (addToCallQueue) {
      MovieClip.axClass._callQueue.push(this);
    }
  ...
  private _buttonFrames: MapObject<number>;
  ...
  get scenes(): ASArray /* flash.display [] */ {
    var scenes = this._scenes ? this._scenes.map(function (x: Scene) {
      return x.clone();
    }) : [];
  ...
  _getAbsFrameNumber(frame: string, sceneName: string): number {
    var navigationModel = MovieClip.axClass.frameNavigationModel;
  ...
  private _gotoFrameAbs(frame: number): void {
    ...
    if (this._allowFrameNavigation) {
      if (MovieClip.axClass.frameNavigationModel === FrameNavigationModel.SWF9) {
        ...
        // display objects.
        DisplayObject.axClass.performFrameNavigation(false, true);
      }
    }
  }
      ...
      if (nextSwfFrame.controlTags) {
        ...
        for (var i = 0; i < tags.length; i++) {
          ...
          if (tag.code === SwfTagCode.CODE_START_SOUND) {
            ...
            soundStarts.push(new SoundStart(tag.soundId, tag.soundInfo));
  ...
  queueAvm1FrameScripts(frame: number, queue: Array<FrameScript>): void {
    ...
    if (this._hasFlags(DisplayObjectFlags.NeedsLoadEvent)) {
      ...
      var handler = function () {
        ...
        this.dispatchEvent(eventClass.getInstance(Event.AVM1_LOAD));
      }.bind(this);
  ...
  addFrameLabel(name: string, frame: number): void {
    ...
    if (!scene.getLabelByName(name, false)) {
      scene.labels.value.push(new FrameLabel(name, frame - scene.offset));
    }
  }

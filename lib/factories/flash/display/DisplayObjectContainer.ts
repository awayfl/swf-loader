import { mixHash } from "../../base/utilities/HashUtilities";
...
export class DisplayObjectContainer extends InteractiveObject {
  ...
  _constructChildren(): void {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      try {
        ...
        warning('caught error executing child constructor in constructChildren: ', e);
      }
      ...
      if (child._hasFlags(DisplayObjectFlags.HasPlaceObjectInitPending)) {
        ...
        try {
          ...
          warning('caught error under DisplayObjectContainer AVM1_INIT event: ', e);
        }
        try {
          child.dispatchEvent(eventClass.getInstance(Event.AVM1_CONSTRUCT));
        } catch (e) {
          warning('caught error under DisplayObjectContainer AVM1_CONSTRUCT event: ', e);
        }
        if (child.hasEventListener(Event.AVM1_LOAD)) {
  ...
  _enqueueFrameScripts() {
    if (this._hasFlags(DisplayObjectFlags.ContainsFrameScriptPendingChildren)) {
      ...
      for (var i = 0; i < children.length; i++) {
        var child = children[i];
        if (DisplayObjectContainer.axIsType(child) ||
            AVM1Movie.axIsType(child)) {
  ...
  _setTabChildren(enable: boolean) {
    ...
    if (old !== enable) {
      this.dispatchEvent(Event.axClass.getInstance(Event.TAB_CHILDREN_CHANGE, true));
    }
  }
  ...
  addChildAt(child: DisplayObject, index: number /*int*/): DisplayObject {
    checkParameterType(child, "child", DisplayObject.axClass);
    ...
    child._addReference();
    child.dispatchEvent(Event.axClass.getInstance(Event.ADDED, true));
    // ADDED event handlers may remove the child from the stage, in such cases
    // we should not dispatch the ADDED_TO_STAGE event.
    if (child.stage) {
      child._propagateEvent(Event.axClass.getInstance(Event.ADDED_TO_STAGE));
    }
    return child;
  }
   ...
   */
  addTimelineObjectAtDepth(child: DisplayObject, depth: number /*int*/) {
  ...
  removeChildAt(index: number): DisplayObject {
    ...
    if (child._hasFlags(DisplayObjectFlags.Constructed)) {
      child.dispatchEvent(Event.axClass.getInstance(Event.REMOVED, true));
      if (this.stage) {
        child._propagateEvent(Event.axClass.getInstance(Event.REMOVED_FROM_STAGE));
      }
   ...
   */
  getTimelineObjectAtDepth(depth: number /*int*/): DisplayObject {
    depth = depth | 0;
    var children = this._children;
    for (var i = 0; i < children.length; i++) {
      var child = children[i];
  ...
  _containsPointImpl(globalX: number, globalY: number, localX: number, localY: number,
    ...
    for (var i = children ? children.length : 0; i--; ) {
      ...
      if (objects.length !== 0) {
        release || assert(InteractiveObject.axIsType(objects[0]));
   ...
   */
  getObjectsUnderPoint(globalPoint: Point): ASArray {
  ...
  swapChildren(child1: DisplayObject, child2: DisplayObject): void {
    ...
    checkParameterType(child1, "child", DisplayObject.axClass);
    checkParameterType(child2, "child", DisplayObject.axClass);

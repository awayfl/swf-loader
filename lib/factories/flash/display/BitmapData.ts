...
import { ColorUtilitites, unpremultiplyARGB, RGBAToARGB, premultiplyARGB } from "../../base/utilities/ColorUtilities";
...
export class BitmapData extends ASObject implements IBitmapDrawable, IRemotable {
  ...
  applySymbol() {
    ...
    this._rect = new Rectangle(0, 0, symbol.width, symbol.height);
   ...
   */
  private _bitmapReferrers: Bitmap [];

  _addBitmapReferrer(bitmap: Bitmap) {
    var index = indexOf(this._bitmapReferrers, bitmap);
    release || assert(index < 0);
    this._bitmapReferrers.push(bitmap);
  }

  _removeBitmapReferrer(bitmap: Bitmap) {
    var index = indexOf(this._bitmapReferrers, bitmap);
    release || assert(index >= 0);
   ...
   */
  private static _temporaryRectangle: Rectangle;

  private _getTemporaryRectangleFrom(rect: Rectangle): Rectangle {
    var r = BitmapData.axClass._temporaryRectangle;
    r.copyFrom(rect);
    return r;
  }
  ...
  _getContentBounds(): Bounds {
    return Bounds.FromRectangle(this._rect);
  }

  /**
   * TODO: Not tested.
   */
  private _getPixelData(rect: Rectangle): Int32Array {
   ...
   * TODO: Not tested.
   */
  private _putPixelData(rect: Rectangle, input: Int32Array): void {
    var r = this._getTemporaryRectangleFrom(this._rect).intersectInPlace(rect);
    if (r.isEmpty()) {
      return;
    }
  ...
  clone(): BitmapData {
    var bd: BitmapData = Object.create(BitmapData.axClass.tPrototype);
    ...
    this._ensureBitmapData();
    bd._id = DisplayObject.getNextSyncID();
  ...
  getPixel32(x: number /*int*/, y: number /*int*/): number /*uint*/ {
    ...
    switch (this._type) {
      ...
      default:
        notImplemented(ImageType[this._type]);
        return 0;
    }
  }
  ...
  copyChannel(sourceBitmapData: BitmapData, sourceRect: Rectangle,
              destPoint: Point, sourceChannel: number /*uint*/, destChannel: number
              /*uint*/): void {
    sourceBitmapData = sourceBitmapData;
    sourceRect = sourceRect;
    destPoint = destPoint;
              ...
              mergeAlpha?: boolean): void
  {
    ...
    if (sourceRect) {
      ...
      sRect = BitmapData.axClass._temporaryRectangle.setEmpty();
    }
  ...
  drawWithQuality(source: IBitmapDrawable, matrix: Matrix = null,
                  colorTransform: ColorTransform = null, blendMode: string = null,
  ...
  fillRect(rect: Rectangle, uARGB: number /*uint*/): void {
    this._ensureBitmapData();
  ...
  hitTest(firstPoint: Point, firstAlphaThreshold: number /*uint*/,
  ...
  merge(sourceBitmapData: BitmapData, sourceRect: Rectangle,
        destPoint: Point, redMultiplier: number /*uint*/, greenMultiplier: number
  ...
  paletteMap(sourceBitmapData: BitmapData, sourceRect: Rectangle,
              destPoint: Point, redArray: any [] = null, greenArray: any [] = null,
  ...
  pixelDissolve(sourceBitmapData: BitmapData, sourceRect: Rectangle,
                destPoint: Point, randomSeed: number /*int*/ = 0,
  ...
  threshold(sourceBitmapData: BitmapData, sourceRect: Rectangle,
            destPoint: Point, operation: string, threshold: number /*uint*/,
  ...
  histogram(hRect: Rectangle = null): GenericVector {
    hRect = hRect;
    release || notImplemented("public flash.display.BitmapData::histogram");
    return;
  }

  encode(rect: Rectangle, compressor: ASObject,
  ...
  private _ensureBitmapData() {
    ...
    if (this._type !== ImageType.PremultipliedAlphaARGB) {
      ColorUtilitites.convertImage(this._type, ImageType.PremultipliedAlphaARGB, this._view,
                                  this._view);
...
export interface IBitmapDataSerializer {
  drawToBitmap(bitmapData: BitmapData, source: IBitmapDrawable,
...
export class BitmapSymbol extends DisplaySymbol
                          implements EagerlyResolvedSymbol {
  width: number;
  height: number;
  syncId: number;
  data: Uint8Array;
  ...
  private sharedInstance: BitmapData;

  constructor(data: SymbolData, sec: ISecurityDomain) {
    super(data, BitmapData.axClass, false);
    this.ready = false;
  }
  ...
  static FromData(data: any, loaderInfo: LoaderInfo): BitmapSymbol {
    ...
    symbol.syncId = DisplayObject.axClass.getNextSyncID();
  ...
  private _unboundResolveAssetCallback(data: any) {
    ...
    this.ready = true;
    if (!data) {
      release || error("Error while decoding image");
      return;
    }

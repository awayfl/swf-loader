...
export class Graphics extends ASObject implements IRemotable {
...
//      leaveTimeline();
   ...
   */
  private _writeBitmapStyle(pathCommand: PathCommand, bitmap: BitmapData,
                            ...
                            skipWrite: boolean): void
  {
    ...
    if (!(BitmapData.axIsType(bitmap))) {
      this.sec.throwError('TypeError', Errors.CheckTypeFailedError, 'bitmap',
                                      'flash.display.BitmapData');
    }
    if (isNullOrUndefined(matrix)) {
      matrix = Matrix.axClass.FROZEN_IDENTITY_MATRIX;
    } else if (!(Matrix.axIsType(matrix))) {

...
export class Font extends ASObject implements Shumway.Remoting.IRemotable {
  ...
  private static _fontsByName: MapObject<Font>;
  ...
  static classInitializer: any = function () {
    this._fonts = [];
    this._fontsBySymbolId = ObjectUtilities.createMap<Font>();
    this._fontsByName = ObjectUtilities.createMap<Font>();
  ...
  static registerFontSymbol(fontMapping: {name: string; style: string; id: number},
    ...
    var syncId = DisplayObject.axClass.getNextSyncID();
  ...
  static resolveFontSymbol(loaderInfo: LoaderInfo, id: number, syncId: number,
                            key: string) {
...
export class FontSymbol extends Symbol implements EagerlyResolvedSymbol {
  name: string;
  bold: boolean;
  italic: boolean;
  codes: number[];
  ...
  constructor(data: SymbolData, sec: ISecurityDomain) {
    super(data, Font.axClass);
  }

  static FromData(data: any, loaderInfo: LoaderInfo): FontSymbol {
    ...
    symbol.syncId = DisplayObject.getNextSyncID();
    return symbol;
  }

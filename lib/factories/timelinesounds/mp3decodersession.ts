...
export var MP3WORKER_PATH: string = './assets/mp3worker.js';
...
export class MP3DecoderSession {
	...
	public static processAll(data: Uint8Array): Promise<{ data: Uint8Array; id3Tags: any; }> {
		...
		const promiseWrapper = new PromiseWrapper<{ data: Uint8Array; id3Tags: any; }>();
		const session = new MP3DecoderSession();
		session.onframedata = function (frameData, channels, sampleRate, bitRate) {
			const needed = frameData.length + bufferPosition;
			if (needed > currentBufferSize) {
				do {
					currentBufferSize *= 2;
				} while (needed > currentBufferSize);
				const newBuffer = new Float32Array(currentBufferSize);
		...
		session.onclosed = function () {
			if (sessionAborted)
				return;
			promiseWrapper.resolve({ data: <any>currentBuffer.subarray(0, bufferPosition), id3Tags: id3Tags });
		};

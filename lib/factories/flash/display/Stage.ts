...
import { DisplayObjectFlags, DisplayObject } from "./DisplayObject";
...
import { release, assert, notImplemented, somewhatImplemented } from "../../base/utilities/Debug";
...
export class Stage extends DisplayObjectContainer {
  ...
  constructor () {
    ...
    this._softKeyboardRect = new Rectangle();
  ...
  set scaleMode(value: string) {
    ...
    if (StageScaleMode.toNumber(value) < 0) {
  ...
  set align(value: string) {
    value = axCoerceString(value);
    var n = StageAlign.toNumber(value);
    release || assert(n >= 0);
    var newValue = StageAlign.fromNumber(n);
  ...
  setStageContainerSize(width: number, height: number, pixelRatio: number) {
    ...
    if (sizeHasChanged) {
      ...
      if (this.scaleMode === StageScaleMode.NO_SCALE) {
        this.dispatchEvent(Event.axClass.getInstance(Event.RESIZE));
      }
    }
  }
  ...
  set focus(newFocus: InteractiveObject) {
  ...
  set quality(value: string)  {
    ...
    if (StageQuality.toNumber(value) < 0) {
  ...
  set displayState(value: string) {
    ...
    if (StageDisplayState.toNumber(value) < 0) {
      value = StageDisplayState.NORMAL;
    }
  ...
  render(): void {
    ...
    DisplayObject.axClass._broadcastFrameEvent(Event.RENDER);
    this._invalidated = false;
  }
  ...
  set contextMenu(value: ContextMenu) {
    this.sec.throwError('IllegalOperationError', Errors.InvalidStageMethodError);
  }
}
...
export class LocalConnection extends EventDispatcher
  ...
  constructor () {
    ...
    var url = new (<any>URL)(getCurrentABC().env.url);
  ...
  connect(connectionName: string): void {
    ...
    var result = LocalConnectionService.createConnection(connectionName, this);
  ...
  send(connectionName: string, methodName: string, ...args): void {
    ...
    var serializedArgs = new ByteArray();
  ...
  public handleMessage(methodName: string, argsBuffer: ArrayBuffer): void {
    ...
    if (!client.axHasPublicProperty(methodName) || forbiddenNames.indexOf(methodName) > -1) {
      ...
      var ba: ByteArray = new ByteArray(argsBuffer);
    ...
    var errorEvent = new AsyncErrorEvent('asyncError', false, false,

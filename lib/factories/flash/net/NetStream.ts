...
import { IDataDecoder } from "../../base/utilities/ArrayUtilities";
...
export class NetStream extends EventDispatcher implements ISoundSource {
  _isDirty: boolean;
  ...
  attachCamera: (theCamera: Camera, snapshotMilliseconds?: number /*int*/) => void;
...
class VideoStream {
  ...
  play(url: string, checkPolicyFile: boolean) {
    ...
    if (isMediaSourceEnabled && typeof MediaSource === 'undefined') {
      warning('MediaSource API is not enabled, falling back to regular playback');
    ...
    if (/\.flv($|\?)/i.test(url)) {
      if (flvOption.value === 'supported') {
        ...
        setTimeout(() => {
          this._netStream.dispatchEvent(new NetStatusEvent(NetStatusEvent.NET_STATUS,
    ...
    if (!forceMediaSource && !isMediaSourceEnabled) {
      ...
      this._url = FileLoadingService.resolveUrl(url);
      return;
    }

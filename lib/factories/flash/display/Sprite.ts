...
import { release, assert, notImplemented, warning } from "../../base/utilities/Debug";
import { LoaderInfo } from "./LoaderInfo";
import { DisplaySymbol, SymbolData } from "../symbol";
import { FrameLabel } from "./FrameLabel";
import { HitTestingResult, HitTestingType, DisplayObject, DisplayObjectFlags } from "./DisplayObject";
...
export class Sprite extends DisplayObjectContainer {
      ...
      switch (tag.code) {
        ...
        case SwfTagCode.CODE_PLACE_OBJECT3:
          ...
          if (placeObjectTag.flags & PlaceObjectFlags.Move) {
            ...
            warning("Warning: Failed to place object at depth " + depth + ".");
            break;
          }

          var symbol: DisplaySymbol = null;
          if (hasCharacter) {
            symbol = <DisplaySymbol>loaderInfo.getSymbolById(placeObjectTag.symbolId);
  ...
  _removeAnimatedChild(child: DisplayObject) {
  ...
  startDrag(lockCenter: boolean = false, bounds: Rectangle = null): void {
    ...
    Mouse.axClass.draggableObject = this;
  }
  stopDrag(): void {
    if (Mouse.axClass.draggableObject === this) {
      Mouse.axClass.draggableObject = null;
      this._dragMode = DragMode.Inactive;
      this._dragDeltaX = 0;
      this._dragDeltaY = 0;
      this._dragBounds = null;
  ...
  startTouchDrag(touchPointID: number /*int*/, lockCenter: boolean = false, bounds: Rectangle = null): void {
...
export class SpriteSymbol extends DisplaySymbol {
  ...
  constructor(data: SymbolData, loaderInfo: LoaderInfo) {
    super(data, MovieClip.axClass, true);
  ...
  static FromData(data: any, loaderInfo: LoaderInfo): SpriteSymbol {
    ...
    for (var i = 0; i < frames.length; i++) {
      ...
      if (frame.labelName) {
        symbol.labels.push(new FrameLabel(frame.labelName, i + 1));
      }
